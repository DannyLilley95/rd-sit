...
local function WakeUp()
	...
	TriggerServerEvent('rd-sit:leavePlace', currentSitCoords)
...
local function sit(object, modelName, data)
	...
	QBCore.Functions.TriggerCallback('rd-sit:getPlace', function(occupied)
		if occupied then
			QBCore.Functions.Notify(Config.Text["Occupied"], 'error')
		else
			lastPos, currentSitCoords = playerPos, objectCoords

			TriggerServerEvent('rd-sit:takePlace', objectCoords)
...
RegisterNetEvent("rd-sit:Sit", function(data)
	local playerPed = PlayerPedId()
